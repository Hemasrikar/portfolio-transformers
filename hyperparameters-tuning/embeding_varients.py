...
class CrossSectionalDataset(Dataset):
    def __init__(self, df, k0_cols, k1_cols, miss_cols, target_cols_list, n_classes, max_firms):
...
def load_split(path, k0_cols, k1_cols, miss_cols, target_cols_list, n_classes, max_firms):
    ...
    return CrossSectionalDataset(df, k0_cols, k1_cols, miss_cols, target_cols_list, n_classes, max_firms)
...
class PortfolioTransformer(nn.Module):
    def __init__(self, config):
        ...
        self.k1_encoder = build_encoder(config.encoding_variant, n_k1, config.d_model,
    ...
    def forward(self, k0, k1, miss):
        ...
        scores_3m = self.head_3m(z).squeeze(-1)
        scores_6m = self.head_6m(z).squeeze(-1)
        scores_12m = self.head_12m(z).squeeze(-1)

        return scores_3m, scores_6m, scores_12m, all_attn


class TopHeavyListNetLoss(nn.Module):
    ...
    def forward(self, pred_scores, true_returns):
        k = min(self.top_k, true_returns.shape[0])
        _, top_indices = torch.topk(true_returns, k=k, dim=-1)

        pred_top = torch.gather(pred_scores, dim=-1, index=top_indices)
        true_top = torch.gather(true_returns, dim=-1, index=top_indices)

        pred_dist = F.log_softmax(pred_top / self.temperature, dim=-1)
        true_dist = F.softmax(true_top / self.temperature, dim=-1)

        return -torch.sum(true_dist * pred_dist, dim=-1)


def compute_multitask_listnet_loss(s3, s6, s12, raw_targets, config):
    criterion = TopHeavyListNetLoss(top_k=config.top_k_listnet, temperature=1.0)

    loss_3m = criterion(s3, raw_targets["target_3m"])
    loss_6m = criterion(s6, raw_targets["target_6m"])
    loss_12m = criterion(s12, raw_targets["target_12m"])

    total_loss = (config.lambda_3m * loss_3m + config.lambda_6m * loss_6m + config.lambda_12m * loss_12m
    )
    return total_loss
...
def objective(trial, variant, gpu_id):
    ...
    config.learning_rate = trial.suggest_float("lr", 5e-6, 1e-3, log=True)
    config.weight_decay = trial.suggest_float("weight_decay", 1e-7, 1e-2, log=True)
    config.top_k_listnet = trial.suggest_categorical("top_k_listnet", [20, 50, 100])
    ...
    config.periodic_num_freq = 16

    if config.d_model >= 128 and config.n_layers >= 3:
        config.learning_rate = min(config.learning_rate, 5e-4)

    print(
        f"Trial {trial.number} on cuda:{gpu_id} | d={config.d_model} heads={config.n_heads} layers={config.n_layers} "
        f"top_k_ln={config.top_k_listnet} lr={config.learning_rate:.1e}"
    )
    sys.stdout.flush()
    ...
    try:
        for epoch in range(1, config.max_epochs + 1):
            ...
            for idx in np.random.permutation(len(train_data)):
                ...
                raw_targets = {k: v.to(target_device, non_blocking=True) for k, v in batch["raw_targets"].items()}

                optimizer.zero_grad(set_to_none=True)
                with torch.autocast(device_type='cuda'):
                    s3, s6, s12, _ = model(k0, k1, miss)
                    loss = compute_multitask_listnet_loss(s3, s6, s12, raw_targets, config)
            ...
            with torch.no_grad():
                for idx in range(len(shared_val_ds)):
                    ...
                    actual_returns_6m = raw_targets["target_6m"]
                    _, s6, _, _ = model(k0, k1, miss)

                    n_assets = s6.shape[0]
                    exec_k = max(1, int(n_assets * 0.10))

                    _, long_idx = torch.topk(s6, k=exec_k, largest=True)
                    _, short_idx = torch.topk(s6, k=exec_k, largest=False)

                    long_ret = actual_returns_6m[long_idx].mean().item()
                    short_ret = actual_returns_6m[short_idx].mean().item()

                    val_monthly_returns.append(long_ret - short_ret)

            returns_np = np.array(val_monthly_returns)
            mean_ret = np.mean(returns_np)
            std_ret = np.std(returns_np, ddof=1)
            val_sharpe = (mean_ret / (std_ret + 1e-8)) * np.sqrt(12)

            print(f"Epoch {epoch:2d} | cuda:{gpu_id} | Val Sharpe: {val_sharpe:.4f} | Avg Monthly Ret: {mean_ret*100:.2f}%")
            sys.stdout.flush()
...
if __name__ == "__main__":
    ...
    print()
    print("Combined portfolio search parameters saved to /kaggle/working/hpt_all_variants.json")
