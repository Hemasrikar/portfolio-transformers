...
A 60-month training window advances one month at a time. The model
retrains from scratch at each step and produces one OOS prediction.
...
n_epochs_per_window = 20     # epochs per window step
...
def main():
	...
	all_months = load_all_months(device)
	...
	print(f'first: {sorted_dates[0].date()}, OOS from: {sorted_dates[window_size].date()}')
